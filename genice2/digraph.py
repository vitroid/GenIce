...
class IceGraph(nx.DiGraph):
    ...
    def isZ22(self):
        ...
        for node in self:
            if (
                len(list(self.successors(node))) != 2
                or len(list(self.predecessors(node))) != 2
            ):
                return False
        return True
...
def traversing_cycles_iter(spaceicegraph, cell, axis):
    ...
    distance = 0
    while distance == 0:
        vertex = random.sample(list(spaceicegraph), 1)[0]
        distance = estimate_edge_length(spaceicegraph, cell, vertex)
    # すべての頂点を順番にあたる。
    for vertex in random.sample(list(spaceicegraph), Nnode):
        apsis = find_apsis(
            list(spaceicegraph), spaceicegraph.coord, cell, distance * 1.3, vertex, axis
        )
        ...
        if path1 is None:
            ...
            continue
        logger.debug(
            "Dipole of the path1: {0}".format(spaceicegraph.dipole_moment(path1))
        )
        path2 = shortest_path(
            spaceicegraph,
            apsis,
        ...
        if path2 is None:
            ...
            continue
        logger.debug(
            "Dipole of the path2: {0}".format(spaceicegraph.dipole_moment(path2))
        )
        ...
        if rr < 0.1:
            logger.debug(
                "Dipole of the harvest: {0}".format(spaceicegraph.dipole_moment(cycle))
            )
            yield cycle


def depolarize_(
    subgraph, coord, immutables=[], pbc=True, cell=np.identity(3), depol="strict"
):
    """
    ...
    spaceicegraph = SpaceIceGraph(subgraph, coord=coord, pbc=pbc, immutables=immutables)
    spaceicegraph.vector_check()

    # TSL
    # defect-defect chains
    defects = []
    for node in spaceicegraph:
    ...
    if len(defects) > 0:
        ...
        while reject_count > 0:
            ...
            if path is None:
                continue
            logger.debug(
                "  Dipole moment = {0}".format(spaceicegraph.dipole_moment(path))
            )
...
def depolarize(
    digraph, coord, immutables=[], pbc=True, cell=np.identity(3), depol="strict"
):
    logger = getLogger()
