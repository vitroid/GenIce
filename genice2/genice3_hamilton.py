# Another plan of reactive GenIce3 using Hamilton.
...
def _assume_water_orientations(
    coord: np.ndarray, digraph: nx.DiGraph, cellmat: np.ndarray, dopants: dict
) -> np.ndarray:
    """
...
def _guest_parser(guest_options: list) -> dict:
...
def _ion_parser(ion_options: list) -> dict:
    result = {}
...
# ============================================================================
# Hamilton関数定義: 依存関係は引数名から自動推論される
# ============================================================================
...
def replica_vector_labels(replica_vectors: np.ndarray) -> dict:
    """レプリカベクトルラベルの生成"""
...
def lattice_sites(
    unitcell: UnitCell, replica_vectors: np.ndarray, replication_matrix: np.ndarray
) -> np.ndarray:
    """格子サイト位置の複製"""
    return replicate_positions(unitcell.waters, replica_vectors, replication_matrix)


def anions(unitcell: UnitCell, replica_vectors: np.ndarray) -> dict:
    """アニオンの複製"""
    anion_dict = {}
...
def cations(unitcell: UnitCell, replica_vectors: np.ndarray) -> dict:
    """カチオンの複製"""
    cation_dict = {}
    Z = len(unitcell.waters)
...
def site_occupants(anions: dict, cations: dict, lattice_sites: np.ndarray) -> list:
    """サイト占有種のリスト"""
...
# ============================================================================
# GenIce3クラス: Hamilton Driverをラップ
# ============================================================================


class GenIce3:
    """
    Hamiltonを使ったreactive GenIce3クラス。
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Hamiltonは型ヒントのない関数を除外するので、
        # ヘルパー関数（guest_parser, ion_parser, replicate_graph, replicate_fixed_edges）は
        # 自動的に除外される
        # アンダースコアで始まる関数も自動的に除外される
        from hamilton.lifecycle import base

        class DictAdapter(base.BaseDoBuildResult):
            """結果を辞書として返すアダプター"""

            def do_build_result(self, **outputs):
                return outputs

        self.driver = driver.Driver({}, current_module, adapter=DictAdapter())
    ...
    def _normalize_result(self, result):
        """
        Hamiltonの結果を正規化して辞書として返す。
        DictAdapterを使っているので通常は辞書のはずだが、
        念のためDataFrameや{'outputs': {...}}形式にも対応。
        """
        # 結果が{'outputs': {...}}形式の場合、'outputs'キーの値を取得
        if isinstance(result, dict) and "outputs" in result and len(result) == 1:
            result = result["outputs"]
        # 通常の辞書として返す（DictAdapterが正しく動作していれば、ここで返される）
        if isinstance(result, dict):
            return result
        # 念のため、DataFrameやその他の形式にも対応（本来は不要なはず）
    ...
    def __getattr__(self, name: str):
        """
        Hamilton関数に対してプロパティアクセスを自動解決する。
        """
        # Hamilton関数を実行
        inputs = self._get_inputs()
        try:
            result = self.driver.execute([name], inputs=inputs)
    ...
    def water_molecules(self, water_model: Molecule):
        ...
        results = self.driver.execute(
            ["lattice_sites", "site_occupants", "orientations", "cell"],
            inputs=inputs,
        )
        results = self._normalize_result(results)
        for site in range(len(results["lattice_sites"])):
            if "water" == results["site_occupants"][site]:
                rel_position = results["lattice_sites"][site]
                orientation = results["orientations"][site]

                sites = water_model.sites @ orientation + rel_position @ results["cell"]
                mols[site] = Molecule(
    ...
    def guest_molecules(self, guests: dict):
        inputs = self._get_inputs()
        results = self.driver.execute(["cages", "cell"], inputs=inputs)
        results = self._normalize_result(results)
        cages_data = results["cages"]
        cell_data = results["cell"]

        randoms = np.random.random(len(cages_data[0]))
        mols = []
        for pos, label, probability in zip(cages_data[0], cages_data[1], randoms):
            accum = 0.0
            if label in guests:
                for guest_spec in guests[label]:
                    ...
                    if accum > probability:
                        mols.append(
                            Molecule(
                                name=molecule.name,
                                sites=molecule.sites + pos @ cell_data,
    ...
    def substitutional_ions(self):
        inputs = self._get_inputs()
        results = self.driver.execute(
            ["anions", "cations", "lattice_sites", "cell"], inputs=inputs
        )
        results = self._normalize_result(results)
        ions = {}
        # ならべかえはここではしない。formatterにまかせる。
        for label, name in results["anions"].items():
            ions[label] = Molecule(
                name=name,
                sites=[results["lattice_sites"][label] @ results["cell"]],
                labels=[name],
                is_water=False,
            )
        for label, name in results["cations"].items():
            ions[label] = Molecule(
                name=name,
                sites=[results["lattice_sites"][label] @ results["cell"]],
                labels=[name],
    ...
    def list_all_reactive_properties(self):
        """
        すべてのreactive property（Hamilton関数）を列挙する。
        """
        # Hamilton Driverから関数リストを取得
        # モジュールから直接取得（Hamilton関数はモジュールレベルで定義されている）
        import sys

        current_module = sys.modules[__name__]
        # Hamilton関数は引数を取る関数として定義されているもの
        hamilton_functions = {
            "cell",
            "replica_vectors",
            "replica_vector_labels",
    ...
    def list_public_reactive_properties(self):
        ...
        return {name for name in all_reactive if name in public_names}
    ...
    # プロパティは__getattr__で自動的に解決されるため、定義不要
    ...
    def to_gro(self, waters: dict, guests: dict, ions: dict):
        ...
        inputs = self._get_inputs()
        results = self.driver.execute(["cell"], inputs=inputs)
        results = self._normalize_result(results)
        cellmat = results["cell"]
...
def main(
    ...
    logger.info(f"     All: {genice.list_all_reactive_properties()}")
    logger.info(f"  Public: {genice.list_public_reactive_properties()}")
    ...
    # 試しにgroファイルにしてみるか。
    with open("genice3_hamilton.gro", "w") as f:
