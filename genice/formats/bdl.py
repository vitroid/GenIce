...
class Formatter(GenIce):
    ...
    def run(self, options):
        ...
        self.stage7B(guests)      #Guest atoms
        #self.logger.info("Total number of atoms: {0}".format(len(self.atoms)))
        self.logger.info("Output in Scigress BDL format.")
        ...
        for gname in self.guestAtoms:
            ...
            s.append("{0:15}                {1:4}  {2:3}  {3:3}".format(gname, self.nGuestAtoms[gname], len(guest.sites), 0))
            for i in range(len(self.guestAtoms[gname])):
                molorder, resname, atomname, position = self.guestAtoms[gname][i]
                q = 0.0  #charge
        ...
        s.append("{0:15}                {1:4}  {2:3}  {3:3}".format(water_type, len(self.reppositions), len(water.sites), 0))
